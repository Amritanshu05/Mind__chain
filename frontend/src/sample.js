  ...
  const reduceTokens = async (amountToReduce) => {
    ...
    if (uContract && amountToReduce) {
      try {
        ...
        // ***REMOVED******REMOVED***-PYUSD***REMOVED******REMOVED***--//
  ...
  return (
    <div className="text-black">
      ...
      {!walletAddress ? (
        ...
        <div>
          <p>Connected: {walletAddress}***REMOVED***
          <p>Token Balance: {tokenBalance} SOUL***REMOVED***
          {((time + 1) < (new Date())) ? (<p>Collect tokens after 24Hrs***REMOVED***) : (<button onClick={earnTokens}>Earn 10 SoulTokens</button>)}
